...
public class LocalAllocNode extends AllocNode {
	...
	public String toString() {
		if (PaddleScene.v().options().number_nodes()) {
			return "LocalAllocNode " + getNumber() + " " + newExpr + " "
					+ method + " type " + getType();
		} else {
			return "LocalAllocNode " + newExpr + " " + method + " type "
					+ getType();
		}
	}
	...
	public LocalAllocNode(Object newExpr, Type t, SootMethod m) {
		super(newExpr, t);
		this.method = m;
		if (m == null)
		...
		containingClass = m.getDeclaringClass();
	}
