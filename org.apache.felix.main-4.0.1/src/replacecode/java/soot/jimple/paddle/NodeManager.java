...
//        if( PaddleScene.v().options().types_for_sites() ) return makeGlobalAllocNode( type, type, m );
    	if( PaddleScene.v().options().types_for_sites() ) return makeGlobalAllocNode( type, type, m, m.getDeclaringClass());
	...
	// by xiaowei zhou, add 'containingClass' parameter, 20111209
    public AllocNode makeGlobalAllocNode( Object newExpr, Type type, SootClass containingClass) {
    	// by xiaowei zhou, , 20111209
        return makeGlobalAllocNode(newExpr, type, null, containingClass);
    }
    
	// by xiaowei zhou, add 'containingClass' parameter, 20111209
    public AllocNode makeGlobalAllocNode( Object newExpr, Type type, SootMethod m, SootClass containingClass ) {
        if( PaddleScene.v().options().types_for_sites() ) newExpr = type;
	AllocNode ret = (AllocNode) valToGlobalAllocNode.get( newExpr );
	if( ret == null ) {
		// by xiaowei zhou, , 20111209
	    valToGlobalAllocNode.put( newExpr, ret = new GlobalAllocNode( newExpr, type, m, containingClass ) );
...
//        Type type = RefType.v( "java.lang.String" );
        Type type = RefType.v( "java.lang.String", ClassLoaders.appCLNum );
...
//            type = RefType.v("java.lang.Object");
            type = RefType.v("java.lang.Object", ClassLoaders.appCLNum);
        }
