...
//                        receivers.add( receiver, source, s, RefType.v("java.security.PrivilegedAction"), sigObjRun, Kind.PRIVILEGED );
                        receivers.add( receiver, source, s, RefType.v("java.security.PrivilegedAction", ClassLoaders.appCLNum), sigObjRun, Kind.PRIVILEGED );
...
//                        receivers.add( receiver, source, s, RefType.v("java.security.PrivilegedExceptionAction"), sigObjRun, Kind.PRIVILEGED );
                        receivers.add( receiver, source, s, RefType.v("java.security.PrivilegedExceptionAction", ClassLoaders.appCLNum), sigObjRun, Kind.PRIVILEGED );
...
//                            if( Scene.v().containsClass("java.lang.ref.Finalizer") ) {
                            if( Scene.v().containsClass("java.lang.ref.Finalizer", ClassLoaders.appCLNum) ) {
                            	// by xiaowei zhou, , 20111214
//                                addEdge( source, s, Scene.v().getSootClass("java.lang.ref.Finalizer").getMethod(sigRegister), Kind.FINALIZE );
                                addEdge( source, s, Scene.v().getSootClass("java.lang.ref.Finalizer", ClassLoaders.appCLNum).getMethod(sigRegister), Kind.FINALIZE );
                            }
                            break;
                        }
                        cl = cl.getSuperclass();
...
//            if( !Scene.v().containsClass( cls ) ) {
            if( !Scene.v().containsClass( cls, src.getDeclaringClass().getDefCLNumber(), 0 ) ) {
                if( options.verbose() ) {
...
//                SootClass sootcls = Scene.v().getSootClass( cls );
                SootClass sootcls = Scene.v().getSootClass( cls, src.getDeclaringClass().getDefCLNumber(), 0 );
...
//        if( Scene.v().containsMethod( methodSig ) ) {
        if( Scene.v().containsMethod( methodSig, src.getDeclaringClass(), 0 ) ) {
        	// by xiaowei zhou, , 20111214
//            addEdge( src, stmt, Scene.v().getMethod( methodSig ), kind );
            addEdge( src, stmt, Scene.v().getMethod( methodSig, src.getDeclaringClass(), 0   ), kind );
        }
    }
