            ...
            if(!success)
                {
                    if(!Scene.v().allowsPhantomRefs())
                    	...
                    	// by xiaowei zhou, comment out, 20120416
            ...
            if(coffiClass.super_class != 0)
                {
                	...
                	// by xiaowei zhou, use bclass's defining class loader to resolve its super class, 20111207
//                    bclass.setSuperclass(SootResolver.v().makeClassRef(superName));
                    bclass.setSuperclass(SootResolver.v().makeClassRefByInitCL(superName, bclass.getDefCLNumber()));
            ...
            for(int i = 0; i < coffiClass.interfaces_count; i++)
                {
                	...
                	// by xiaowei zhou, use bclass's defining class loader to resolve its interfaces, 20111207
//                    SootClass interfaceClass = SootResolver.v().makeClassRef(interfaceName);
                    SootClass interfaceClass = SootResolver.v().makeClassRefByInitCL(interfaceName, bclass.getDefCLNumber());
        ...
        for(int i = 0; i < coffiClass.fields_count; i++){
            ...
            Type fieldType = jimpleTypeOfFieldDescriptor(fieldDescriptor, bclass);
            
//        	// by xiaowei zhou, , 20111207
//            if(fieldType instanceof RefType) {
//            	((RefType)fieldType).setInitCL(bclass.getDefCLNumber());
//            }
        ...
        for(int i = 0; i < coffiClass.methods_count; i++){
            ...
            {
            	// by xiaowei zhou, , 20111207
                Type[] types = jimpleTypesOfFieldOrMethodDescriptor(methodDescriptor, bclass);
                        ...
                        for(int k = 0; k < exceptions.number_of_exceptions; k++)
{
                        	...
                        	// by xiaowei zhou, use bclass's defining class loader to resolve its exceptions, 20111207
//                            method.addExceptionIfAbsent(SootResolver.v().makeClassRef(exceptionName));
                            method.addExceptionIfAbsent(SootResolver.v().makeClassRefByInitCL(exceptionName, bclass.getDefCLNumber()));
                        }
                    }
                    ...
                    for(int k = 0; k < coffiClass.constant_pool_count; k++) {
                        if(coffiClass.constant_pool[k] instanceof CONSTANT_Class_info)
                            {
                                ...
                                if(name.startsWith("["))
                                	// by xiaowei zhou, , 20111207
                                    references.add(jimpleTypeOfFieldDescriptor(desc, bclass));
                                else
                                    references.add(name);
                            }
                        ...
                        || coffiClass.constant_pool[k] instanceof CONSTANT_InterfaceMethodref_info) {
                            ...
                            Type[] types = jimpleTypesOfFieldOrMethodDescriptor(
                                cp_info.getTypeDescr(coffiClass.constant_pool,k), bclass);
	...
	// by xiaowei zhou, add a parameter of containing class to retrieve its defining class loader, 20111207
    Type jimpleReturnTypeOfMethodDescriptor(String descriptor, SootClass containingClass)
    {
    	// by xiaowei zhou, , 20111207
        Type[] types = jimpleTypesOfFieldOrMethodDescriptor(descriptor, containingClass);
...
*/
	...
	// by xiaowei zhou, add a parameter of containing class to retrieve its defining class loader, 20111207
    public Type[] jimpleTypesOfFieldOrMethodDescriptor(String descriptor, SootClass containingClass)
    {
            ...
            while(p<d.length) {
                ...
                // Skip parenthesis
                    ...
                    case 'L':
                    	...
                    	// by xiaowei zhou, , 20111207
                        baseType = RefType.v(className, containingClass.getDefCLNumber(), 0);
                        p = index+1;
                        break swtch;
                    case 'S':
                        baseType = ShortType.v();
...
//    public Type jimpleTypeOfFieldDescriptor(String descriptor)
    public Type jimpleTypeOfFieldDescriptor(String descriptor, SootClass containingClass)
    {
        boolean isArray = false;
        int numDimensions = 0;
        Type baseType;
        ...
        // Determine base type
            ...
            else if(descriptor.startsWith("L"))
            {
            	...
            	// by xiaowei zhou, , 20111207
                baseType = RefType.v(className.replace('/', '.'), containingClass.getDefCLNumber(), 0);
            }
