...
public class CFG {
    ...
    public boolean jimplify(cp_info constant_pool[],int this_class, JimpleBody listBody)
   {
    	...
    	// by xiaowei zhou, , 20111212
        SootClass containingClass = jmethod.getDeclaringClass();
...
//            Type thisType = RefType.v(jmethod.getDeclaringClass().getName());
			Type thisType = RefType.v(containingClass.getName(),
					containingClass.getDefCLNumber());
...
//        jimplify(constant_pool,this_class);
        jimplify(constant_pool,this_class, containingClass);
     ...
     * @see Stmt
     */
	// by xiaowei zhou, add a parameter 'containingClass', 20111212
    void jimplify(cp_info constant_pool[],int this_class, SootClass containingClass)
    {
        Code_attribute codeAttribute = method.locate_code_attribute();
...
//                            exception = cm.getSootClass(name);
                            exception = cm.getSootClass(name, containingClass.getDefCLNumber(), 0);
                        }
                        else
                        	// by xiaowei zhou, , 20111213
//                            exception = cm.getSootClass("java.lang.Throwable");
                        	exception = cm.getSootClass("java.lang.Throwable", ClassLoaders.appCLNum);
...
//                                    handlerInstructionToException.get(s).getName()));
                            	TypeStack exceptionTypeStack = (TypeStack.v()).push(RefType.v(
                                        handlerInstructionToException.get(s).getName(), containingClass.getDefCLNumber(), 0));
...
//                            statementsForIns, b);
                 	  generateJimple(ins, instructionToTypeStack.get(ins),
                            instructionToPostTypeStack.get(ins), constant_pool,
                            statementsForIns, b, containingClass);
                    else
...
//			exception = cm.getSootClass("java.lang.Throwable");
	...
	if (Options.v().keep_line_number())
	{
		...
		if (element instanceof LineNumberTable_attribute)
		{
			...
			// by xiaowei zhou, set the has line number field of sootclass to true, 20120412
		    containingClass.setHasLineNumber(true);
	...
	// by xiaowei zhou, add a parameter 'containingClass', 20111213
     OutFlow processFlow(Instruction ins, TypeStack typeStack,
        cp_info[] constant_pool, SootClass containingClass)
    {
        int x;
        x = ((ins.code))&0xff;
...
//                ((Instruction_Ldc1)ins).arg_b, typeStack, jmethod);
            return processCPEntry(constant_pool,
                    ((Instruction_Ldc1)ins).arg_b, typeStack, jmethod, containingClass);
...
//                ((Instruction_intindex)ins).arg_i, typeStack, jmethod);
            return processCPEntry(constant_pool,
                    ((Instruction_intindex)ins).arg_i, typeStack, jmethod, containingClass);

         case ByteCode.ACONST_NULL:
        		// by xiaowei zhou, , 20111212
//            typeStack = typeStack.push(RefType.v("java.lang.Object"));
            typeStack = typeStack.push(RefType.v("java.lang.Object", ClassLoaders.appCLNum));
            break;
...
//            typeStack = typeStack.push(RefType.v("java.lang.Object"));
        ...
        case ByteCode.ANEWARRAY:
        {
            ...
            if(name.startsWith("[")) {
            	...
            	// by xiaowei zhou, , 20111213
                baseType = Util.v().jimpleTypeOfFieldDescriptor(baseName, containingClass);
            } else {
            	// by xiaowei zhou, , 20111213
                baseType = RefType.v(name, containingClass.getDefCLNumber(), 0);
            }
        ...
        case ByteCode.MULTIANEWARRAY:
        {
            ...
            ArrayType arrayType = (ArrayType)
                Util.v().jimpleTypeOfFieldDescriptor(arrayDescriptor, containingClass);
...
//                ((Instruction_Putfield)ins).arg_i));
            Type type = byteCodeTypeOf(jimpleTypeOfFieldInFieldRef(cm, constant_pool,
                    ((Instruction_Putfield)ins).arg_i, containingClass));
...
//                ((Instruction_Getfield)ins).arg_i));
            Type type = byteCodeTypeOf(jimpleTypeOfFieldInFieldRef(cm, constant_pool,
                    ((Instruction_Getfield)ins).arg_i, containingClass));
...
//                ((Instruction_Putstatic)ins).arg_i));
            Type type = byteCodeTypeOf(jimpleTypeOfFieldInFieldRef(cm, constant_pool,
                    ((Instruction_Putstatic)ins).arg_i, containingClass));
...
//                ((Instruction_Getstatic)ins).arg_i));
            Type type = byteCodeTypeOf(jimpleTypeOfFieldInFieldRef(cm, constant_pool,
                    ((Instruction_Getstatic)ins).arg_i, containingClass));
...
//                constant_pool, iv.arg_i));
            Type returnType = byteCodeTypeOf(jimpleReturnTypeOfMethodRef(cm,
                    constant_pool, iv.arg_i, containingClass));
...
//            Type type = RefType.v(getClassName(constant_pool, ((Instruction_New)ins).arg_i));
            Type type = RefType.v(getClassName(constant_pool, ((Instruction_New)ins).arg_i), containingClass.getDefCLNumber(), 0);
         ...
         case ByteCode.CHECKCAST:
         {
            ...
            if(className.startsWith("["))
                ...
                ((Instruction_Checkcast)ins).arg_i), containingClass);
            else
            	// by xiaowei zhou, , 20111213
//                castType = RefType.v(className);
            	castType = RefType.v(className, containingClass.getDefCLNumber(), 0);
    ...
    private Type jimpleTypeOfFieldInFieldRef(Scene cm,
        cp_info[] constant_pool, int index, SootClass containingClass)
    {
...
//        return Util.v().jimpleTypeOfFieldDescriptor(fieldDescriptor);
        return Util.v().jimpleTypeOfFieldDescriptor(fieldDescriptor, containingClass);
    }
...
//        return Util.v().jimpleReturnTypeOfMethodDescriptor(methodDescriptor);
        return Util.v().jimpleReturnTypeOfMethodDescriptor(methodDescriptor, containingClass);
    }
...
//        return Util.v().jimpleReturnTypeOfMethodDescriptor(methodDescriptor);
    ...
    private OutFlow processCPEntry(cp_info constant_pool[],int i,
                            TypeStack typeStack,
                            SootMethod jmethod, SootClass containingClass)
    {
...
//            typeStack = typeStack.push(RefType.v("java.lang.String"));
        	typeStack = typeStack.push(RefType.v("java.lang.String", ClassLoaders.appCLNum));
            ...
            if (name.charAt(0) == '['){
                ...
                switch (typeIndicator){
                    ...
                    case 'L': baseType = RefType.v(
                                      name.substring(dim+1, name.length()-1), containingClass.getDefCLNumber(), 0);
                              break;
...
//                typeStack = typeStack.push(RefType.v(name));
                typeStack = typeStack.push(RefType.v(name, containingClass.getDefCLNumber(), 0));
            }
        }
        else
                            ...
                            SootMethod jmethod, List<Stmt> statements)
   {
    ...
    void generateJimple(Instruction ins, TypeStack typeStack, TypeStack postTypeStack,
        ...
        List<Stmt> statements, BasicBlock basicBlock, SootClass containingClass)
   {
      Value[] params;
...
//                baseType = Util.v().jimpleTypeOfFieldDescriptor(getClassName(constant_pool, ((Instruction_Anewarray)ins).arg_i));
            	baseType = Util.v().jimpleTypeOfFieldDescriptor(getClassName(constant_pool, ((Instruction_Anewarray)ins).arg_i), containingClass);
            else
            	// by xiaowei zhou, , 20111213
//                baseType = RefType.v(baseName);
            	baseType = RefType.v(baseName, containingClass.getDefCLNumber(), 0);
...
//               ArrayType jimpleType = (ArrayType) Util.v().jimpleTypeOfFieldDescriptor(mstype);
               ArrayType jimpleType = (ArrayType) Util.v().jimpleTypeOfFieldDescriptor(mstype, containingClass);
...
//            Type fieldType = Util.v().jimpleTypeOfFieldDescriptor(fieldDescriptor);
            Type fieldType = Util.v().jimpleTypeOfFieldDescriptor(fieldDescriptor, containingClass);
                
        	// by xiaowei zhou, , 20111213
//            SootClass bclass = cm.getSootClass(className);
            SootClass bclass = cm.getSootClass(className, containingClass.getDefCLNumber(), 0);
...
//                Type[] types = Util.v().jimpleTypesOfFieldOrMethodDescriptor(methodDescriptor);
                Type[] types = Util.v().jimpleTypesOfFieldOrMethodDescriptor(methodDescriptor, containingClass);
...
//                ((Instruction_New)ins).arg_i));
            SootClass bclass = cm.getSootClass(getClassName(constant_pool,
                    ((Instruction_New)ins).arg_i), containingClass.getDefCLNumber(), 0);

        	// by xiaowei zhou, , 20111213
//            stmt = Jimple.v().newAssignStmt(Util.v().getLocalForStackOp(listBody, postTypeStack,
//                postTypeStack.topIndex()), Jimple.v().newNewExpr(RefType.v(bclass.getName())));
            stmt = Jimple.v().newAssignStmt(Util.v().getLocalForStackOp(listBody, postTypeStack,
                    postTypeStack.topIndex()), Jimple.v().newNewExpr(RefType.v(bclass.getName(), containingClass.getDefCLNumber(), 0)));
            break;
         }
...
//                ((Instruction_Instanceof)ins).arg_i));
            	checkType = Util.v().jimpleTypeOfFieldDescriptor(getClassName(constant_pool,
                    ((Instruction_Instanceof)ins).arg_i), containingClass);
            else
            	// by xiaowei zhou, , 20111213
//                checkType = RefType.v(className);
            	checkType = RefType.v(className, containingClass.getDefCLNumber(), 0);
