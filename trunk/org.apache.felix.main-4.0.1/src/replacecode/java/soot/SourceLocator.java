...
import cn.iscas.tcse.osgiclassloadanalyzer.pointstoanalysis.forsoot.OrdinaryClassPath;
...
//    public ClassSource getClassSource(String className) 
    public ClassSource getClassSource(String className, Integer intendedDefCL)
    {
    	
    	// by xiaowei zhou, , 20111206
    	if(intendedDefCL.equals(ClassLoaders.appCLNum)) {
            if( appClassPath == null ) {
                appClassPath = new OrdinaryClassPath(Scene.v().getSootClassPath());
            }
    		nowClassPath = appClassPath;
    	} else {
    		nowClassPath = Scene.v().getCustomCP(intendedDefCL);
			if (!customCPs.containsKey(intendedDefCL)) {
				customCPs.put(intendedDefCL, nowClassPath);
			}
    	}
    	//        if( appClassPath == null ) {
//            appClassPath = explodeClassPath(Scene.v().getSootClassPath());
//        }
...
//    private List<String> classPath;
    private ISootClassPath appClassPath;
    private Map<Integer, ISootClassPath> customCPs = new HashMap<Integer, ISootClassPath>();
    private ISootClassPath nowClassPath;

	// by xiaowei zhou, comment out since no one calls it, 20111206
//    public List<String> classPath() { return appClassPath; }
    ...
    public List<String> sourcePath() {
    	// by xiaowei zhou, temporarily kill it, so we cannot use soot.JavaClassSource class, 20111206
//        if( sourcePath == null ) {
//            sourcePath = new ArrayList<String>();
//            for (String dir : appClassPath) {
...
//        return sourcePath;
    ...
    public static class FoundFile {
    	// by xiaowei zhou, change to public, 20111206
        public FoundFile( ZipFile zipFile, ZipEntry entry ) {
        ...
        public InputStream inputStream() {
            try {
            	// by xiaowei zhou, change to support url, 20111214
...
//        }
//        return null;
		return nowClassPath.lookupInClassPath(fileName);
